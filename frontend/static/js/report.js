...
report = (function () {
    ...
    var _getCss = function () {
        ...
        let classIcons = {};
        document.querySelectorAll('.report .custom-icon').forEach((dataviz) => {
            let res = dataviz.className.match(/^(.* )?(icon-([^-]+)-([^ ]+))( .*)?$/);
            if (res) {
                classIcons[ res[2] ] = '.' + res[2] + ' { background-image: url(/static/picto/' + res[3] + '/' + res[4] + '.svg); }\n';
                classIcons[ res[2] ]+= 'img.' + res[2] + ' { background: transparent; content: url(/static/picto/' + res[3] + '/' + res[4] + '.svg); }';
            }
        });
